$('<div>').attr('id', 'panelbox').prependTo(document.body);
/* $('<div>').attr('id','panel').appendTo('#engine');
$('<div>').attr('id','code').appendTo('#engine');*/
/* looper number */
var panelid = 0;
/* create div tag panelbox and assign it with unique id */
$('<div>').attr('id', 'node' + panelid).appendTo('#panelbox');
/* creates form for buttons to create elements */
$('<form>').attr('id', 'node' + panelid).appendTo('#panelbox' + panelid)
panelid++;
$('<hr />').appendTo('#panelbox');
...
$('#input').click(function () {
  inputnum++;
  $('<input type="text">').attr('id', 'input' + inputnum).attr('placeholder',
